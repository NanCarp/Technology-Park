...
public class ParkManageService {
	...
	public static Page<Record> getEmpList(Integer pageno, int pagesize) {
		return Db.paginate(pageno, pagesize,
				"select e.id,e.name,e.empno,e.phone," + "d.department_name,p.position_name,e.status,r.role_name",
	...
	// 修改以及添加员工信息
	public static boolean saveEmp(Integer id, String name, String password, int empno, String phone, int department_id,
			int position, int role_id, int status) {
	...
	public static Page<Record> getBuilding(Integer pageno, int pagesize, Integer fstate, String buildno) {
		String sqlExceptSelect = " from t_building b LEFT JOIN t_building_nature n "
				+ "ON b.nature = n.id LEFT JOIN t_building_number u " + "ON b.building_no = u.id where 1=1";
	...
	// 修改以及添加楼宇信息
	public static boolean saveBuild(Integer id, String name, int nature, int floor_no, int building_no, int total_area,
			int usable_area, int status) {
		Record record = new Record();
		record.set("name", name);
	...
	/*********************** 区域管理 ************************/
	// 区域管理
	public static Page<Record> getArea(Integer pageno, int pagesize, Integer fstate, String direction, String areaname,
			String company_name) {
	...
	// 修改以及添加区域信息
	public static boolean saveArea(Integer id, String area_name, String direction, String area_no, String building_no,
	...
	/*********************** 缴费管理 ************************/
	// 查询缴费记录
	public static Page<Record> getPayment(Integer pageno, int pagesize, Integer year, String quarterly,
			String company_name) {
	...
	/*********************** 安全检查管理 ************************/
	// 安全管理检查记录
	public static Page<Record> getSafeInspect(Integer pageno, int pagesize, String sdate, Integer rectification,
			String company_name) {
	...
	// 修改以及添加安全检查记录
	public static boolean saveSafetyRecord(Integer id, int company_id, String company_name, Date check_time,
			String examiner, Date rectification_time, String rectification_man, String supervision_personnel,
			String hidden_danger, String performance, String remarks, int is_rectification) {
		Record record = new Record();
		record.set("company_id", company_id);
		record.set("company_name", company_name);
	...
	// 导出安全管理检查记录列表
	public static boolean getSafeRecordForExcel(HttpServletResponse response, String sdate, Integer rectification,
			String company_name) {
		...
		response.addHeader("Content-Disposition",
				"attachment;filename=" + EncordUtil.toUtf8String("安全管理检查" + date + "月记录统计") + ".xls");
	...
	public static void excWord(HttpServletResponse response, HttpServletRequest request, Integer id)
		...
		DocUtil.toSafe(request, DocUtil.WORD_TEEPLATE, dataMap);
		try {
